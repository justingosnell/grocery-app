...
async function checkServerAvailable(timeoutMs = 3000) {
    ...
    if (!serverAvailable) {
        showToast('Server offline: login and cloud sync disabled. Local lists still work.', 'info');
    } else {
        initSocket();
    }
...
function setServerUIByAvailability() {
    // Hide auth UI entirely if server is not available
...
function setupEventListeners() {
    if (loginLink) {
        loginLink.addEventListener('click', () => {
            ...
            if (modal) handleModal(modal, true);
        });
    }
    if (accountNavBtn) {
