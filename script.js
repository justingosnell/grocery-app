...
console.log('About to declare cancelAddBtn...');
...
// saveListBtn removed - now using clearListBtn as save button
...
// fabAdd removed - no longer needed
...
// saveListBtn check removed - now using clearListBtn as save button
if (!savedListsContainer) missingElements.push('savedListsContainer');
if (!messageAlert) missingElements.push('messageAlert');
...
if (!toggleSavedPanel) missingElements.push('toggleSavedPanel');
if (!savedPanel) missingElements.push('savedPanel');
...
function showAlert(message, type = 'success') {
    if (!messageAlert) {
        console.error('messageAlert element not found');
        return;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add similar setupClearButton calls for itemInput, emojiInput, savedSearchInput
});
...
function renderEmptyCurrentList() {
    if (currentListElement) {
        currentListElement.innerHTML = `
            <li class="p-6 text-center text-gray-600 flex flex-col items-center">
                <span class="text-lg">Your grocery list is empty </span>
            </li>
        `;
    }
...
function handleAddItem(e) {
    ...
    console.log('Item details:', { itemName, quantity, emoji }); // Debug log
...
function handleSavedListActions(e) {
    ...
    if (action === 'view') {
        ...
        // Close the saved drawer if open
        const overlay = document.getElementById('savedPanelOverlay');
        const panel = document.getElementById('savedPanel');
        if (panel && !panel.classList.contains('hidden')) {
            const btn = document.getElementById('toggleSavedPanel');
            if (btn) {
...
function updateSaveButtonState() {
    const hasListName = listNameInput && listNameInput.value.trim() !== '';
    const hasItems = currentList.length > 0;
    if (clearListBtn) {
        clearListBtn.disabled = !hasListName || !hasItems;
        // Update button text based on state
        if (!hasItems) {
            clearListBtn.textContent = 'Add items to save';
        } else if (!hasListName) {
            clearListBtn.textContent = 'Enter name to save';
        } else {
...
function setupEventListeners() {
    ...
    // Note: Original floating add button functionality removed - now handled by scroll-to-top button
    ...
    console.log('Setting up form event listener, addItemForm:', addItemForm); // Debug log
    if (addItemForm) {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
            e.preventDefault();
            if (clearListBtn && !clearListBtn.disabled) handleSaveList();
        }
    ...
    // Saved panel toggle (mobile) + overlay to close
    if (toggleSavedPanel && savedPanel) {
        const overlay = document.getElementById('savedPanelOverlay');
        const setOpen = (open) => {
            toggleSavedPanel.setAttribute('aria-expanded', String(open));
            // Update tab chevron direction instead of label
            const icon = toggleSavedPanel.querySelector('span');
            if (icon) icon.textContent = open ? '<' : '>';
            savedPanel.classList.toggle('hidden', !open);
        };
        // Toggle via button
    ...
    // clearListBtn now functions as save button
    if (clearListBtn) {
        clearListBtn.addEventListener('click', handleSaveList);
    }
    
    // Clear list link functionality
    if (clearListLink) {
        clearListLink.addEventListener('click', (e) => {
            e.preventDefault();
            if (currentList.length === 0) {
            ...
            handleModal(clearListModal, true);
        });
    }
